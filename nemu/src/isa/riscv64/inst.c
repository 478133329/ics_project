...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT((int64_t)BITS(src1, 31, 0) >> (imm & 0x3f), 32));
  // 寄存器移位指令也是只取低六位。
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(rd) = SEXT((int64_t)BITS(src1, 31, 0) >> (src2 & 0x3f), 32));
  ...
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, R(rd) = ((int64_t)src1 * src2) >> 32);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (src1 * src2) >> 32);
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, SEXT(BITS(src1 * src2, 31, 0), 32));
